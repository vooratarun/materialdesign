...
public class PlaceProvider extends ContentProvider {
	...
	public static final String AUTHORITY = "info.androidhive.materialdesign.util.PlaceProvider";
    ...
    String mKey = "key=AIzaSyDpkRYf8Jd-dnkF_-2VkiE36q-awxZGo-8";
	
	// Defines a set of uris allowed with this content provider
	private static final UriMatcher mUriMatcher = buildUriMatcher();	
	
	private static UriMatcher buildUriMatcher() {
		Log.d("mytag","Uri Matcher");
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case SEARCH:
			...
			Log.d("mytag","Json String search block" + jsonString);
			try {
		...
		case DETAILS :
			...
			for(int j=0;j<detailsList.size();j++){
				...
				Log.d("mytag", hMapDetails.get("formatted_address") + " " + hMapDetails.get("lat") +  hMapDetails.get("lng"));
    ...
    private String downloadUrl(String strUrl) throws IOException{
        ...
        try{
				...
				Log.d("mytag"," Data Received" + data);
    ...
    private String getPlaceDetails(String reference){
		...
		Log.d("mytag","getReferences Url " + url);
		try {
			data = downloadUrl(url);
		} catch (IOException e) {
			e.printStackTrace();
